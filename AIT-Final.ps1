...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        Title="Android Imaging Tool" Height="300" Width="500"
...
$settingsFile = "$env:APPDATA\AndroidImagingTool\folderpaths.json"
...
function Show-MissingPathDialog {
    ...
    $xamlInput = @"
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        Title="Android Imaging Tool" Height="330" Width="500"
...
function Find-Adb {
    $explicitPath = "$env:USERPROFILE\Android Imaging Tool\platform-tools-latest-windows\platform-tools\adb.exe"
...
$adbPath = Find-Adb
if (-not $adbPath) {
    new-item -ItemType Directory "$env:USERPROFILE\Android Imaging Tool\platform-tools-latest-windows\platform-tools" -Force
    copy-item "$adbFolder\*" "$env:USERPROFILE\Android Imaging Tool\platform-tools-latest-windows\platform-tools" -Recurse
    $adbPath = "$env:USERPROFILE\Android Imaging Tool\platform-tools-latest-windows\platform-tools\adb.exe"
}
...
$proceedBtn.Add_Click({
    ...
    $model = deviceModel
    $logFile = Join-Path $env:USERPROFILE "AppData\Roaming\AndroidImagingTool\image_log.txt"
...
#### - ET60 / Android 13
    if ($model.Model -eq "ET60") {
        ...
        $scriptBlock = { # Image Logic
            ...
            Write-Status "Configuring Android settings..."
            ...
            Get-Success { & $adbPath shell "mkdir -p /storage/emulated/0/Android/data/de.proglove.connect/files" } "Insight Mobile folder created." "Failed to create IM folder" | Out-Null
            Get-Success { & $adbPath push $proPath "/storage/emulated/0/Android/data/de.proglove.connect/files" } "Insight Mobile configuration file pushed." "Failed to push IM file" | Out-Null
            ...
            if ($syncHash.Result) {
                ...
                Write-Status "Image failed. Please check log file located here:`n$env:APPDATA\AndroidImagingTool\"
            }
        }
...
#### - MC3300x / Android 11
    if ($model.Model -eq "MC3300x") {
        ...
        $scriptBlock = { # Image Logic
            ...
            Write-Status "Configuring Velocity..."
            Get-Success { & $adbPath shell "mkdir /storage/emulated/0/Android/data/com.wavelink.velocity" } "Velocity folder created." "Failed to create Velocity folder" | Out-Null
            Get-Success { & $adbPath shell "mkdir /storage/emulated/0/Android/data/com.wavelink.velocity/files" } "Velocity subfolder created." "Failed to create Velocity subfolder" | Out-Null
            Get-Success { & $adbPath push $radioFile "/storage/emulated/0/Android/data/com.wavelink.velocity/files" } "Velocity RADIO file pushed." "Failed to push Velocity file" | Out-Null

            Write-Status "Importing DataWedge profile..."
            Get-Success { & $adbPath push $dw3300xPath "/storage/emulated/0/Download" } "Insight Mobile folder created." "Failed to create IM folder" | Out-Null
            Get-Success { & $adbPath shell "mv /storage/emulated/0/Download/dwprofile_Velocity.db /enterprise/device/settings/datawedge/autoimport" } "Insight Mobile configuration file pushed." "Failed to push IM file" | Out-Null
            Start-Sleep -Seconds 2
            Get-Success { & $adbPath shell "chmod 777 /enterprise/device/settings/datawedge/autoimport/dwprofile_Velocity.db" } "Insight Mobile folder created." "Failed to create IM folder" | Out-Null
...
#### - MC33 / Android 8.1
    if ($model.Model -eq "MC33") {
        ...
        $scriptBlock = { # Image Logic
            ...
            Write-Status "Setting home application..."
            Get-Success { & $adbPath shell "am start -a android.intent.action.MAIN -n com.symbol.enterprisehomescreen/.HomeScreenActivity" } "Home app set." "Failed to set home app" | Out-Null
